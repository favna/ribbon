...
export default class TimerAddCommand extends Command {
  ...
  public async run(msg: CommandoMessage, {
    interval, timerChannel, content, members,
  }: TimerAddArgs) {
    ...
    try {
      conn.prepare(oneLine`
        INSERT INTO "${msg.guild.id}" (interval, channel, content, lastsend, members)
        VALUES ($interval, $channel, $content, $lastsend, $members);`)
        .run({
          content,
          interval,
