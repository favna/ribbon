...
export default class SelfRolesCommand extends Command {
    constructor(client: CommandoClient) {
        super(client, {
            ...
            description:
                'Sets the self assignable roles for the server members, to be used by the `iam` command',
    ...
    public run(
        ...
        if (!roles.length) {
            description =
                'self assignable roles have been removed, members who previously gave themselves a role will keep that role!';
            msg.guild.settings.remove('selfroles');
        } else {
            roles.forEach(role => roleIDs.push(role.id));
            roles.forEach(role => roleNames.push(role.name));
            description = oneLine`self assignable roles have been set to ${roleNames
                .map(role => `\`${role}\``)
                .join(', ')}`;
