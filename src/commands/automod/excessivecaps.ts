...
export default class ExcessiveCapsCommand extends Command {
        ...
        super(client, {
            ...
            args: [
                ...
                },
                {
                    key: 'threshold',
                    prompt:
                        'After how much percent of caps should a message be deleted?',
                    type: 'string',
                    default: '60',
                    validate: (t: string) => {
                        if (/(?:[%])/.test(t)) {
                    ...
                    parse: (t: string) =>
                        /(?:[%])/.test(t) ? Number(t.slice(0, -1)) : Number(t),
                },
                {
                    key: 'minlength',
                    prompt:
                        'What should the minimum length of a message be before it is checked?',
                    type: 'integer',
                    default: 10,
                },
            ],
    ...
    public run(
        msg: CommandoMessage,
        {
            ...
            threshold,
            minlength,
        }: { option: boolean; threshold: number; minlength: number }
    ) {
        startTyping(msg);
        ...
        ecfEmbed
            ...
            .setDescription(
                stripIndents`**Action:** Excessive Caps filter has been ${
                    option ? 'enabled' : 'disabled'
                }
      ${
          option
              ? `**Threshold:** Messages that have at least ${threshold}% caps will be deleted`
              : ''
      }
      ${
          option
              ? `**Minimum length:** Messages of at least ${minlength} are checked for caps`
              : ''
      }
      ${
