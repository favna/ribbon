...
export default class SlowmodeCommand extends Command {
        ...
        super(client, {
            ...
            args: [
                ...
                },
                {
                    key: 'within',
                    prompt:
                        'Within how many seconds should duplicate messages be checked?',
                    type: 'integer',
                    default: 10,
                },
            ],
    ...
    public run(
        msg: CommandoMessage,
        { option, within }: { option: boolean; within: number }
    ) {
        startTyping(msg);
        ...
        const modlogChannel = msg.guild.settings.get(
            'modlogchannel',
            msg.guild.channels.find((c: TextChannel) => c.name === 'mod-logs')
                ? msg.guild.channels.find(
                      (c: TextChannel) => c.name === 'mod-logs'
                  ).id
                : null
        );
        const options = {
        ...
        slEmbed
            ...
            .setDescription(
                stripIndents`**Action:** Slowmode has been ${
                    option ? 'enabled' : 'disabled'
                }
      ${
          option
              ? `**Info:** Slow mode enabled. Members can 1 message per ${within} second(s)`
              : ''
      }
      ${
