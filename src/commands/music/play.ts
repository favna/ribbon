...
export default class PlaySongCommand extends Command {
  ...
  public async run (msg: CommandoMessage, { videoQuery }: {videoQuery: string}) {
    ...
    if (videoQuery.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
      ...
      // @ts-ignore
      if (!this.queue.get(msg.guild.id).playing) this.play(msg.guild, this.queue.get(msg.guild.id, queue).songs[0]);
  ...
  get votes () {
    if (!this.queueVotes) {
      // @ts-ignore
      this.queueVotes = this.client.registry.resolveCommand('music:skip').votes;
    }
