...
export default class UserInfoCommand extends Command {
    ...
    public run (msg: CommandoMessage, { member }: { member: GuildMember }) {
        ...
        uinfoEmbed
            ...
            .addField('Role(s)', member.roles.size > 1 ? arrayClean(null, member.roles.map(r => r.name).filter(name => name !== '@everyone')).join(' | ') : 'None')
            .addField('Account created at', moment(member.user.createdAt)
                .format('MMMM Do YYYY [at] HH:mm:ss [UTC]Z'), true)
