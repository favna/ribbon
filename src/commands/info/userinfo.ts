...
export default class UserInfoCommand extends Command {
    ...
    public run(msg: CommandoMessage, { member }: { member: GuildMember }) {
        ...
        uinfoEmbed
            ...
            .addField(
                ...
                member.user.presence.status !== 'dnd'
                    ? capitalizeFirstLetter(member.user.presence.status)
                    : 'Do Not Disturb',
                true
            )
            .addField(
                member.user.presence.activity
                    ? capitalizeFirstLetter(member.user.presence.activity.type)
                    : 'Activity',
                member.user.presence.activity
        ...
        member.roles.size >= 1
            ? uinfoEmbed.setFooter(
                  `${member.displayName} has ${member.roles.size - 1} role(s)`
              )
