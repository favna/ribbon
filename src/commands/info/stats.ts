...
export default class RibbonStatsCommand extends Command {
    ...
    public async run(msg: CommandoMessage): Promise<any> {
        ...
        statsEmbed
            ...
            .setAuthor(
                'Ribbon Bot Stats',
                'https://favna.xyz/images/appIcons/ribbon.png'
            )
            ...
            .addField(
                'Platform',
                this.fetchPlatform(process.platform.toLowerCase()),
                true
            )
            .addField(
                'Memory Usage',
                `${roundNumber(process.memoryUsage().heapUsed / 10485.76) /
                    100} MB`,
                true
            )
            .addField(
                'Invite Me',
                '[Click Here](https://favna.xyz/redirect/ribbon)',
                true
            )
            .addField(
                'Source',
                '[Available on GitHub](https://github.com/favna/ribbon)',
                true
            )
            .addField(
                'Support',
                '[Server Invite](https://favna.xyz/redirect/server)',
                true
            )
            .addField(
                'Uptime',
                moment
                    ...
                    .format('DD [days], HH [hours and] mm [minutes]'),
                true
            )
            .addField(
                'Current server time',
                moment().format('MMMM Do YYYY [|] HH:mm.ss [UTC]ZZ'),
                false
            )
            .addField(
                '\u200b',
                ...
                If you ever forget the command prefix, just use \`${
                    this.client.user.tag
                } prefix\``
            )
            .setFooter(
                `Ribbon | ${moment().format(
        ...
        speed.on('data', (data: any) => {
            ...
            statsEmbed
                .addField(
                    'Download Speed',
                    `${roundNumber(data.speeds.download, 2)} Mbps`,
                    true
                )
                .addField(
                    'Upload Speed',
                    `${roundNumber(data.speeds.upload, 2)} Mbps`,
                    true
                )
                .addField(
                    'Current server time',
