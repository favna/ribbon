...
export default class ShipCommand extends Command {
    ...
    public async run(
        msg: CommandoMessage,
        { romeo, juliet }: { romeo: any; juliet: any }
    ) {
        startTyping(msg);
        romeo =
            romeo !== 'random' ? romeo.user : msg.guild.members.random().user;
        juliet =
            juliet !== 'random' ? juliet.user : msg.guild.members.random().user;

        const avaOne = await Jimp.read(
            romeo.displayAvatarURL({ format: 'png' })
        );
        const avaTwo = await Jimp.read(
            juliet.displayAvatarURL({ format: 'png' })
        );
        const boat = new MessageEmbed();
        const canvas = await Jimp.read(384, 128);
        const heart = await Jimp.read(
            'https://favna.xyz/images/ribbonhost/heart.png'
        );
        ...
        boat.attachFiles([embedAttachment])
            ...
            .setDescription(
                oneLine`I call it...
              ${romeo.username.substring(
                  0,
                  roundNumber(romeo.username.length / randLengthRomeo)
              )}${juliet.username.substring(
                    roundNumber(juliet.username.length / randLengthJuliet)
                )}! 😘`
            )
