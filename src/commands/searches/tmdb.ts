...
export default class MovieCommand extends Command {
  ...
  public async run (msg: CommandoMessage, { name }: {name: string}) {
    try {
      ...
      const movieStats = await fetch(`https://api.themoviedb.org/3/movie/${movieList.results[0].id}?${stringify({ api_key: process.env.MOVIEDB_API_KEY })}`);
      ...
      movieEmbed
        ...
        .addField('Genres', movie.genres.length ? movie.genres.map((genre: IGenre) => genre.name).join(', ') : 'None on TheMovieDB', false);
