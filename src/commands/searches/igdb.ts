...
export default class IGDBCommand extends Command {
  ...
  public async run (msg: CommandoMessage, { game }: {game: string}) {
    ...
    try {
      ...
      const headers = {headers: { Accept: 'application/json', 'user-key': process.env.IGDB_API_KEY }};

      const gameSearch = await fetch(`https://api-endpoint.igdb.com/games/?${stringify({
      ...
      const coverImg = await gameInfo[0].cover.url.includes('http') ? gameInfo[0].cover.url : `https:${gameInfo[0].cover.url}`;
      const releaseDate = moment(gameInfo[0].release_dates[0].date).format('MMMM Do YYYY');

      const companyFetch = await fetch(`https://api-endpoint.igdb.com/companies/${gameInfo[0].developers.join()}?${stringify({
        fields: [ 'name' ].join(),
        limit: 1,
        offset: 0,
      })}`, headers);
      ...
      const genreFetch = await fetch(`https://api-endpoint.igdb.com/genres/${gameInfo[0].genres.join()}?${stringify({
        fields: ['name'].join(),
      })}`, headers);
      const genreInfo = await genreFetch.json();

      const platformFetch = await fetch(`https://api-endpoint.igdb.com/platforms/${gameInfo[0].platforms.join()}?${stringify({
        fields: ['name'].join(),
      })}`, headers);
      ...
      gameEmbed
        ...
        .addField(`${gameInfo[0].pegi ? 'PEGI' : 'ESRB'} rating`, gameInfo[0].pegi ? gameInfo[0].pegi.rating : gameInfo[0].esrb.rating, true)
