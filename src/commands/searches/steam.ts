...
export default class SteamCommand extends Command {
            ...
            args: [
                {
                    key: 'game',
                    prompt:
                        'Which game do you want to find on the steam store?',
                    type: 'string',
                },
            ],
        });
    ...
    public async run(msg: CommandoMessage, { game }: { game: string }) {
        try {
            ...
            const steamSearch = await fetch(
                `http://store.steampowered.com/search/?${stringify({
                    category1: 998,
                    term: game,
                })}`
            );
            ...
            const gameID = $(
                '#search_result_container > div:nth-child(2) > a:nth-child(2)'
            )
                .attr('href')
                .split('/')[4];
            const steamFetch = await fetch(
                `https://store.steampowered.com/api/appdetails?${stringify({
                    appids: gameID,
                ...
                })}`,
                {
                    headers: {
                        'User-Agent':
                            'Ribbon Discord Bot (https://github.com/ribbon)',
                    },
                }
            );
            ...
            steamEmbed
                ...
                .addField(
                    ...
                    steamData.price_overview
                        ? `${currencymap(
                              steamData.price_overview.currency
                          )}${this.insert(
                              steamData.price_overview.final.toString(),
                              ','
                          )}`
                        : 'Free',
                    true
                ...
                .addField(
                    'Age requirement',
                    steamData.required_age !== 0
                        ? steamData.required_age
                        : 'Everyone / Not in API',
                    true
                )
            ...
            return msg.embed(
                steamEmbed,
                `http://store.steampowered.com/app/${steamData.steam_appid}/`
            );
        } catch (err) {
            deleteCommandMessages(msg, this.client);
            stopTyping(msg);
