...
export default class TVCommand extends Command {
  ...
  public async run (msg: CommandoMessage, { name }: {name: string}) {
    try {
      ...
      const showStats = await fetch(`https://api.themoviedb.org/3/tv/${showList.results[0].id}?${stringify({ api_key: process.env.MOVIEDB_API_KEY })}`);
      ...
      showEmbed
        ...
        .addField('Genres', show.genres.length ? show.genres.map((genre: IGenre) => genre.name).join(', ') : 'None on TheMovieDB');
