...
export default class AbilityCommand extends Command {
    ...
    public run(msg: CommandoMessage, { ability }: { ability: string }) {
        try {
            ...
            const abilitySearch = aliasSearch.length
                ? abilityFuse.search(aliasSearch[0].ability)
                : abilityFuse.search(ability);
            ...
            abilityEmbed
                ...
                .addField(
                    'Description',
                    abilitySearch[0].desc
                        ? abilitySearch[0].desc
                        : abilitySearch[0].shortDesc
                )
                .addField(
                    'External Resource',
                    oneLine`
                    [Bulbapedia](http://bulbapedia.bulbagarden.net/wiki/${capitalizeFirstLetter(
                        abilitySearch[0].name.replace(/ /g, '_')
                    )}_(Ability\\))
                    |  [Smogon](http://www.smogon.com/dex/sm/abilities/${abilitySearch[0].name
                        .toLowerCase()
                        .replace(/ /g, '_')})
                    |  [PokémonDB](http://pokemondb.net/ability/${abilitySearch[0].name
                        .toLowerCase()
                        .replace(/ /g, '-')})
			    `
                );
            ...
            return msg.embed(
                abilityEmbed,
                `**${capitalizeFirstLetter(abilitySearch[0].name)}**`
            );
        } catch (err) {
            deleteCommandMessages(msg, this.client);
            stopTyping(msg);
            ...
            if (
                ...
                return msg.reply(
                    stripIndents`no ability found for \`${ability}\``
                );
            }
            ...
            channel.send(stripIndents`
                ...
                }> Error occurred in \`ability\` command!
