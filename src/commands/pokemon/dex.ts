    ...
    public run (msg: CommandoMessage, { pokemon, shines }: { pokemon: string, shines: boolean }) {
        try {
            ...
            const pokeData: IPokeData = {
                ...
                genders: '',
                sprite: '',
                tier: smogonFormats.find((s: any) => s.name === poke.species.toLowerCase()
                    .replace(/(-|%| )/gm, '')).tier,
            };
        ...
        → ${pokeData.evos} **(${poke.evoLevel})**`;
            ...
            if (poke.evos) {
                pokeData.evos = oneLine`${pokeData.evos} → ${poke.evos.map((entry: string) => `\`${capitalizeFirstLetter(entry)}\` (${pokeFuse.search(entry)[0].evoLevel})`)
                    .join(', ')} `;
            ...
            → ${pokeFuse.search(poke.evos[0])[0].evos.map((entry: string) => `\`${capitalizeFirstLetter(entry)}\` (${pokeFuse.search(entry)[0].evoLevel})`)
                            .join(', ')}`;
                    }
                }
            }
            ...
            dexEmbed
                ...
                .addField('Base Stats', Object.keys(poke.baseStats)
                ...
                .addField('External Resource', oneLine`${poke.num >= 0 ? `
                    [Bulbapedia](http://bulbapedia.bulbagarden.net/wiki/${capitalizeFirstLetter(poke.species)
