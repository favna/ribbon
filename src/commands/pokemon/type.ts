...
export default class TypeCommand extends Command {
    constructor(client: CommandoClient) {
        super(client, {
            ...
            memberName: 'type',
            description:
                'Get type matchup for a given type or type combination',
    ...
    public run(msg: CommandoMessage, { types }: { types: Array<string> }) {
        try {
            startTyping(msg);
            const atk: any = {
            ...
            embed
                ...
                .addField(
                    ...
                    Supereffective against: ${atk.doubleEffectiveTypes
                        .map((el: string) => `${el} (x4)`)
                        .concat(
                            atk.effectiveTypes.map((el: string) => `${el} (x2)`)
                        )
                        .join(', ')}
                    ...
                    ${
                        atk.effectlessTypes.length
                            ? `Doesn't affect: ${atk.effectlessTypes.join(
                                  ', '
                              )}`
                            : ''
                    }
                ...
                .addField(
                    ...
                    Vulnerable to: ${def.doubleEffectiveTypes
                        .map((el: string) => `${el} (x4)`)
                        .concat(
                            def.effectiveTypes.map((el: string) => `${el} (x2)`)
                        )
                        .join(', ')}
                    ...
                    ${
                        def.effectlessTypes.length
                            ? `Not affected by: ${def.effectlessTypes.join(
                                  ', '
                              )}`
                            : ''
                    }
            ...
            channel.send(stripIndents`
                ...
                }> Error occurred in \`type\` command!
            ...
            return msg.reply(stripIndents`An error occurred matching those types and I notified ${
