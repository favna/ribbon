...
export default class LearnCommand extends Command {
    constructor(client: CommandoClient) {
        super(client, {
            ...
            memberName: 'learn',
            description:
                'Displays how a Pokemon can learn given moves, if at all',
            ...
            args: [
                {
                    key: 'pokemon',
                    prompt:
                        'For which Pokemon should I check move learnability?',
                    type: 'string',
                    validate: (v: string) => {
    ...
    public run(
        msg: CommandoMessage,
        {
            ...
            moves,
            gen = 7,
        }: { pokemon: string; moves: any; gen?: number }
    ) {
        try {
            startTyping(msg);
            if (/(?:--gen)/i.test(moves)) {
                ...
                moves =
                    ...
                    moves.substring(
                        moves.indexOf('--gen') + '--gen'.length + 1
                    );
            }
            ...
            moves.forEach((move: any) => {
                if (move in learnset) {
                    ...
                    methods.forEach(method => {
                        switch (
                            method.slice(1, 2) // tslint:disable-line:switch-default
                        ) {
                            case 'L':
                                response.push(
                                    `${pokemon} **__can__** learn ${move} by level up at level ${method.slice(
                                        2
                                    )}`
                                );
                                break;
                            case 'V':
                                response.push(
                                    `${pokemon} **__can__** learn ${move} through virtual console transfer`
                                );
                                break;
                            case 'T':
                                response.push(
                                    `${pokemon} **__can__** learn ${move} through transferring from a previous generation`
                                );
                                break;
                            case 'M':
                                response.push(
                                    `${pokemon} **__can__** learn ${move} through TM`
                                );
                                break;
                            case 'E':
                                response.push(
                                    `${pokemon} **__can__** learn ${move} as Egg Move`
                                );
                                break;
                            case 'S':
                                response.push(
                                    `${pokemon} **__can__** learn ${move} through an event`
                                );
                                break;
                            case 'D':
                                response.push(
                                    `${pokemon} **__can__** learn ${move} through Dream World`
                                );
                                break;
                        }
                    });
            ...
            learnEmbed
                ...
                .setAuthor(
                    ...
                    `https://favna.xyz/images/ribbonhost/pokesprites/regular/${pokemon
                        .toLowerCase()
                        .replace(/ /g, '')}.png`
                )
                .setDescription(
                    response.length
                        ? response.join('\n')
                        : stripIndents`${capitalizeFirstLetter(
                              pokemon
                          )} cannot learn ${moves
                              .map((val: string) => `\`${val}\``)
                              .join(', ')} in generation ${gen}.
            ...
            channel.send(stripIndents`
                ...
                }> Error occurred in \`learn\` command!
